    ...
    for entryUp in dbupdate:
        ...
        if any(str(parts[0]) in s for s in dbcompare):
            if entryUp in dbcompare:  # same
                ...
                oldFileHash = \
                dbcompare[dbcompare.index(next((s for s in dbcompare if parts[0] in s), None))].split(";")[1].replace(
                    "sha256:", "")
                newFileHash = parts[1].replace("sha256:", "")
                oldStatHash = \
                dbcompare[dbcompare.index(next((s for s in dbcompare if parts[0] in s), None))].split(";")[2].replace(
                    "stat:", "")
